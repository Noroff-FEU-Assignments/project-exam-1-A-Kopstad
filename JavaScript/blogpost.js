...
//function to fetch the specific blog post
async function fetchBlogPost() {
  ...
  try {
    ...
    errorMessage.textContent =
      "Oops! We're having trouble loading the post. Please check your connection and try again.";
    throw error;
  } finally {
    loader.classList.add("hidden");
  }
}
//function to display the blogpost, post.
function displayBlogPost(displayPost) {
  try {
    const blogPost = document.querySelector(".grid-column-1");
    const parser = new DOMParser();
    const doc = parser.parseFromString(
      displayPost.content.rendered,
      "text/html"
    );
...
//hides the modal on the .close div
document.querySelector(".close").addEventListener("click", function () {
  document.getElementById("myModal").style.display = "none";
});
//hides the modal when clicking outside the img. 
document.getElementById("myModal").addEventListener("click", function (event) {
  if (event.target == this) {
    this.style.display = "none";
  }
});
//fetchblogpost call then display the post. 
fetchBlogPost()
